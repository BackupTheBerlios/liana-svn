...
public class Money implements Cloneable, Serializable
{
		...
		throws InvalidScaleFactorException
	{
		...
		(scale < 0) // 0, 1, or
			...
			throw new InvalidScaleFactorException("Invalid scale factor: "
					+ scale + " (must be 0, 1, or 2)");
	...
	public Money(String string, Locale locale) throws ParseException
	{
		...
		try {
			...
			try {
				// Try to parse it as a number without currency symbols
				value = new BigDecimal(string);
			} catch (NumberFormatException nfe) {
				// Unable to parse the string into a number.
				throw nfe;
			}
		}
	}
	...
	public static Money getNonNullableMoney(ResultSet result, int index)
		throws SQLException
	{
		Money m = getNullableMoney(result, index);
		if (m == null) {
			m = CURRENCY_ZERO;
		}
		return m;
	...
	public static Money getNonNullableMoney(CallableStatement stmt, int index)
		throws SQLException
	{
		Money m = getNullableMoney(stmt, index);
		if (m == null) {
			m = CURRENCY_ZERO;
		}
		return m;
	...
	public static Money getNullableMoney(ResultSet result, int index)
		throws SQLException
	{
		double d = result.getDouble(index);
		if (result.wasNull()) {
			return null;
		} else {
			return new Money(d);
	...
	public static Money getNullableMoney(CallableStatement stmt, int index)
		throws SQLException
	{
		double d = stmt.getDouble(index);
		if (stmt.wasNull()) {
			return null;
		} else {
			return new Money(d);
	...
	public static void setNullableMoney(PreparedStatement stmt, int index, Money m)
		throws SQLException
	{
		setNullableMoney(stmt, index, m, Types.DOUBLE);
	}
	
	public static void setNullableMoney(
			PreparedStatement stmt, int index, Money m, int sqlType
		)
		throws SQLException
	{
		if (m == null) {
			...
			stmt.setDouble(index, m.toDouble());
		}
	}
    ...
    /**
     * Compares this <tt>Money</tt> with the specified
     * <tt>Money</tt>.  Two <tt>Money</tt> objects that are
     ...
     *
     * @param  val <tt>Money</tt> to which this <tt>Money</tt> is 
     *         to be compared.
     * @return -1, 0, or 1 as this <tt>Money</tt> is numerically 
